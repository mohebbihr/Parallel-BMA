// BMAOpenMP-SSE4D.c : Defines the entry point for the console application.
...
void init(int argc, char *argv[])
{
	...
	if (argc < 3){
		printf("Usage: ./BMAOpenMP-SSELess filename length -s NumberofSerialInputs -t NumberofThreads  -b debugLvl\n");
		printf("\tto generate random string: ./BMAOpenMP-SSELess randGen length\n");
		exit(1);
	}
	if (argc > 3){
		i = 3;
...
void bitBMASSESerial(int input_index){
	...
	while (n[input_index]<lenS){	
	  ...
	  if(n[input_index] <=128){
		...
		if(n[input_index]+3 <lenS){
			n3 = lenS-4-n[input_index];
		}else n3 = 0;

		if(n[input_index]+2 <lenS){
			n2 = lenS-3-n[input_index]; 
		}else n2 = 0;

		if(n[input_index]+1 <lenS){
			n1 = lenS-2-n[input_index];
		}else n1 = 0;
		
		//q = (lenS-1-n) >> 5;	
		tt = _mm_set_epi32(n3, n2, n1, lenS-1-n[input_index]);
...
{		
	  ...
	  #pragma omp parallel private(i,nThreads,tid,my_first,my_last)
	  {
		  ...
		  /*for(i=0; i<(nSerialInputs/nThreads); i++){
			...
			bitBMASSESerial(i*nThreads+tid); 
		  }
		 //printf("after first loop, tid:%d\n",tid);
		  ...
		  //for(i=j; i<nSerialInputs; i++){
			...
			bitBMASSESerial(i+tid); 
		  //}*/
	  }

	  //printf("after par loop \n");
