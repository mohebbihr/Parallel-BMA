// BMASSE-4D.c: This implementation of BMA use SSE instructions to predict the d value in greedy manner. If we predict the values correctly
// the we can skip the four next iteration. So, in the best case senario the number of iterations of main loop would be reduced by factor of 4.
...
void init(int argc, char *argv[])
{
	...
	if (argc < 3){
		printf("Usage: ./BMASSE-4D filename length -b debugLvl\n");
		printf("\tto generate random string: ./BMASSE-4D randGen length\n");
		exit(1);
	}
	if (argc > 3){
		i = 3;
	...
	rS = (unsigned char*)aligned_malloc(sizeof(unsigned char)*lenS,16);
	if (randGen){
...
void bitBMASSE(void){	
	...
	while (n<lenS){	
	  ...
	  if( n<=128){
	    ...
	    for(i=0;i<(bitLenC>>2);i++){
			t = i<<2;										
		    dSSE = _mm_xor_si128(dSSE,_mm_and_si128(_mm_set_epi32(bitC[t+3],bitC[t+2],bitC[t+1],bitC[t]),_mm_set_epi32(bitRS[r][q+t+3],bitRS[r][q+t+2],bitRS[r][q+t+1],bitRS[r][q+t])));			
...
{		
  ...
  fprintf(fp,"BMASSE-ToomanyIterations, result of executing for len: %d\n",lenS);
  fprintf(fp,"t1: %f\n",t1);
  fclose(fp);
