/* This is the library of bch.h for the user space. 
*  binary BCH encoding/decoding library
*
*/
#ifndef _BCH_H
#define _BCH_H
...
/**
* memcpy using SSE2 instructions
*/
...
inline unsigned int gf_mul(struct bch_control *bch, unsigned int a,
				  unsigned int b)
{
...
inline unsigned int gf_sqr(struct bch_control *bch, unsigned int a)
{
	return a ? bch->a_pow_tab[mod_s(bch, 2*bch->a_log_tab[a])] : 0;
}

inline unsigned int gf_div(struct bch_control *bch, unsigned int a,
				  unsigned int b)
{
...
inline unsigned int gf_inv(struct bch_control *bch, unsigned int a)
{
	return bch->a_pow_tab[GF_N(bch)-bch->a_log_tab[a]];
}

inline unsigned int a_pow(struct bch_control *bch, int i)
{
	return bch->a_pow_tab[modulo(bch, i)];
}

inline int a_log(struct bch_control *bch, unsigned int x)
{
	return bch->a_log_tab[x];
}

inline int a_ilog(struct bch_control *bch, unsigned int x)
{
...
inline int modulo(struct bch_control *bch, unsigned int v)
{
 ...
 */
inline int mod_s(struct bch_control *bch, unsigned int v)
{
	const unsigned int n = GF_N(bch);
	return (v < n) ? v : v-n;
}
...
/*static void gf_poly_copy_sse(struct gf_poly *dst, struct gf_poly *src)
{
	sse_memcpy(dst, src, GF_POLY_SZ(src->deg));
}*/
					    ...
					    const unsigned int *syn)
{
	...
	/*__m512i zero, temp1, temp2, dAVX;	
	__mmask16 mask, mask2;
	zero = _mm512_xor_epi32(zero,zero);
	uint16_t * arrtmp = (uint16_t*) aligned_malloc(16 * sizeof(uint16_t),64);
	uint16_t * arrd = (uint16_t*) aligned_malloc(16 * sizeof(uint16_t),64);*/
	...
	for (i = 0; (i < t) && (elp->deg <= t); i++) {
		if (d) {
			...
			// replace this loop with AVX instructions
			...
			// begining of AVX loop			
			/*for (j = 0; j <(pelp->deg>>4); j++) {
				t2 = j<<4;				
				temp1 = _mm512_load_epi32((void *)(pelp->c[j] + t2)); // pelp->c[j]					
				temp2 = _mm512_load_epi32((void *)(pelp->c[t2 + k]));  // pelp->c[j+ k]				
				mask = _mm512_cmp_epi32_mask(temp1,zero,4); // create mask for if (pelp->c[j])				
				//l = a_log(bch, pelp->c[j]); we can not use shuffle here, because the size of table is too big (32768 * 4 Bytes)
				for(index = 0; index < 16; index++){
					l = a_log(bch, pelp->c[t2 + index]);
					arrtmp[index] = a_pow(bch, tmp+l);
				}													
				temp2 = _mm512_mask_xor_epi32(zero,mask,temp2,_mm512_load_epi32((void *)(arrtmp)));								
			}
			t2 = (pelp->deg>>4)<<4;
			for (j = t2; j <= pelp->deg; j++) {
				if (pelp->c[j]) {
					l = a_log(bch, pelp->c[j]);
					elp->c[j+k] ^= a_pow(bch, tmp+l);
				}
		...
		if (i < t-1) {
			...
			/*for (j = 1; j <(pelp->deg>>4); j++){
				//d ^= gf_mul(bch, elp->c[j], syn[2*i+2-j]);
				t2 = j<<4;	
				temp1 = _mm512_load_epi32((void *)(pelp->c[j] + t2)); //  elp->c[j]
				for(index = 0; index < 16; index++){					
					arrtmp[index] = syn[2*i+2-(t2 + index)];					
					arrd[index] = d;
				}
				dAVX = _mm512_load_epi32((void *)(arrd));
				temp2 = _mm512_load_epi32((void *)(arrtmp)); // syn[2*i+2-j]
				temp1 = _mm512_cmp_epi32_mask(temp1,temp2); // a&b 
				mask = _mm512_cmp_epi32_mask(temp1,zero,4); // mask a&b
...
/*static int compute_error_locator_polynomial_SSE(struct bch_control *bch,
					    const unsigned int *syn)
{
	const unsigned int t = GF_T(bch);
	const unsigned int n = GF_N(bch);
	...
	for (i = 0; (i < t) && (elp->deg <= t); i++) {
		if (d) {
			...
			if (tmp > elp->deg) {
				elp->deg = tmp;
				gf_poly_copy_sse(pelp, elp_copy);
				pd = d;
				pp = 2*i;
			}
		}
...
*/
void encode_bch_unaligned(struct bch_control *bch,
	       ...
	       const unsigned int *syn, unsigned int *errloc)
{
	...
	//err = compute_error_locator_polynomial_SSE(bch, syn);	
...
/*int decode_bch_SSE(struct bch_control *bch, const uint8_t *data, unsigned int len,
	       const uint8_t *recv_ecc, const uint8_t *calc_ecc,
	       const unsigned int *syn, unsigned int *errloc)
{
	const unsigned int ecc_words = BCH_ECC_WORDS(bch);
