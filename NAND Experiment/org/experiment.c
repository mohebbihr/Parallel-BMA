...
void main(int argc, char *argv[]){						
	...
	int sec, usec;
	long int iterations = 100; // 1
	long int nSerialInputs;
	int bufsize = 2048; // 32
	int eccsize = 2048; //32
	int eccbytes = 47; // 3
	int oobsize = 128; //8
	...
	printf("Experiment starts with - m: %d, t: %d, pagesize(Bytes): %d, numpage: %d, number of pages with error: %d\n", m,t,eccsize,numpage, numerrpages);
	...
	for(i=0; i<numpage; i++){
		err = nand_bch_correct_data(mtd,(void *)&error_data[i * bufsize],(void *)&error_ecc[i * eccsize],(void *)&correct_ecc[i * eccsize]);
		//if(err == t)
		//	printf("i: %d , err: %d \n", i, err);
	}
